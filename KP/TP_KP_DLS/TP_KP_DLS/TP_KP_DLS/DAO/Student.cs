...
namespace TP_KP_DLS.DAO
{
    public class Student:DAO
    {
        ...
        public List<Subject> Get_All_Subject( string Id)//(AccountController UserId)
        {
            ...
            try
            {
                SqlCommand command = new SqlCommand("SELECT ID_Subject_Name FROM Subject WHERE ID_Subject IN (SELECT Access_ID_Subject FROM  Access WHERE Access_ID_User ='" + Id +"')", Connection);
                SqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
